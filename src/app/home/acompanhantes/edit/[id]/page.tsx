...
import { convertTimestampToDate, stringToDate } from "@/services/functions";
...
const Page = () => {
    ...
    const [model, setModel] = useState<Acompanhante>()
    ...
    useEffect(() => {
        setTimeout(async () => {
            const get = await api.acompanhante.getById(id)

            if (get.error) {

            } else if (get.model) {
                setModel(get.model)
                let times: any = get.model.nascimento
                let nascimento = convertTimestampToDate(times)
                setDateIso(nascimento.toISOString().split('T')[0])

                //Midias
                const perfilFetch = (await api.acompanhante.getPhotoFiles(get.model.midia.perfil)).list
                const bannerFetch = (await api.acompanhante.getPhotoFiles(get.model.midia.banner)).list
                const cardFetch = (await api.acompanhante.getPhotoFiles(get.model.midia.card)).list
                const comparacaoFetch = (await api.acompanhante.getVideoFiles(get.model.midia.comparacao)).list
                console.log(get.model.midia.perfil)
                //Convertendo StorageReference[] do firebase para file
                perfilFetch.forEach(async (itemRef: any) => {
                ...
                setQuinzeMin(get.model.valores.quinze_min)
                setTrintaMin(get.model.valores.meia_hora)
                setUmaHora(get.model.valores.uma_hora)
                setDuasHoras(get.model.valores.duas_horas)
                setQuatroHoras(get.model.valores.quatro_horas)
                setDiaria(get.model.valores.diaria)
                setPernoite(get.model.valores.pernoite)
                setDiariaViagem(get.model.valores.diaria_viagem)
                setLoading(false)
            }
        })
    }, [])
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        if (perfilFile && bannerFile && cardFile && comparacaoFile) {
            ...
            const insert = await api.acompanhante.update(acompanhante, id)
            ...
            if (insert.error) {
                ...
                for (const file of [perfilFile, bannerFile, cardFile, comparacaoFile]) {
                    const uploadMidia = await api.acompanhante.uploadMidia(file, id)
                ...
                if (links.length == 4) {
                    ...
                    const update = await api.acompanhante.updateMidiaAfterRegister(id, links[0], links[1], links[2], links[3])
    ...
    return (
        <div className='flex flex-col w-full flex-1'>
            ...
            <div className="flex flex-col w-full flex-1">
                ...
                <div className='flex flex-col w-full flex-1 relative px-4'>
                    ...
                    {model &&
                        <>
                            <h1 className='text-3xl font-semibold my-12'>Editar Acompanhante</h1>
                            <form className="flex flex-col w-full mb-12" onSubmit={
                                e => {
                                    e.preventDefault()
                                    handleSubmit(e)
                                }
                                ...
                                <div className='flex-col gap-5 w-full' style={{ display: formScreen == 'dados' ? 'flex' : 'none' }}>
                                    ...
                                    <fieldset className="flex flex-col md:flex-row gap-3">
                                        ...
                                        <TextField
                                            required
                                            defaultValue={model.nome_social}
                                    ...
                                    <fieldset className="flex flex-col md:flex-row gap-3">
                                        ...
                                        <TextField
                                            defaultValue={model.comportamento}
                                            required
                                    ...
                                    <fieldset className="flex flex-col md:flex-row gap-3">
                                        <TextField
                                            defaultValue={model.estiloCabelo}
                                            required
                                    ...
                                    <fieldset className="flex flex-col md:flex-row gap-3">
                                        <Editor type="Html" htmlString={model.descricao} />
                                    </fieldset>
                                    <span className='font-extralight text-sm'>Localização e Valores</span>
                                    <fieldset className="flex flex-col md:flex-row gap-3">
                                        <TextField
                                            defaultValue={model.local}
                                            required
                                            label='Local'
                                            name='local'
                                            size='small'
                                    ...
                                    <fieldset className="grid lg:grid-cols-2 gap-3">
                                        <Price
                                            ...
                                            name={'valor-despesa'}
                                            muiStyle
                                            value={model.valores.quinze_min}
                                        />
                                        <Price
                                            edit={true}
                                            fn={setTrintaMin}
                                        ...
                                        <Price
                                            ...
                                            name={'valor-despesa'}
                                            muiStyle
                                            value={model.valores.duas_horas}
                                        />
                                        <Price
                                            edit={true}
                                            fn={setQuatroHoras}
                                            ...
                                            name={'valor-despesa'}
                                            muiStyle
                                            value={model.valores.quatro_horas}
                                        />
                                        <Price
                                            edit={true}
                                            fn={setDiaria}
                                        ...
                                        <Price
                                            ...
                                            name={'valor-despesa'}
                                            muiStyle
                                            value={model.valores.diaria_viagem}
                                        />
                                    </fieldset>
                                    <span className='text-sm font-extralight ttext-center'>Caso não realize o serviço o valor deverá estar zerado</span>
                                    <hr />
                                    <span className="flex justify-between">
                                        <Link href='/home/acompanhantes' className="px-5 py-2 border rounded-md hover:shadow-sm text-red-500">Cancelar</Link>
                                ...
                                <div className='flex flex-col gap-5 w-full' style={{ display: formScreen == 'servicos' ? 'flex' : 'none' }}>
                                    ...
                                    <fieldset className=' w-full'>
                                        <FormGroup sx={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', fontSize: '12px', width: '100%' }}>
                                            <FormControlLabel control={<Checkbox name="acessorios_eroticos" defaultChecked={model.servicos.penetração_com_acessorios} />} label="Acessórios eróticos" sx={{ width: '100%' }} />
                                            <FormControlLabel control={<Checkbox name="beijo_grego" defaultChecked={model.servicos.beijo_grego} />} label="Beijo grego" />
                                            <FormControlLabel control={<Checkbox name="beijo_na_boca" defaultChecked={model.servicos.beijo_na_boca} />} label="Beijo na boca" />
                                            <FormControlLabel control={<Checkbox name="bondage" defaultChecked={model.servicos.bondage} />} label="Bondage" />
                                            <FormControlLabel control={<Checkbox name="chuva_dourada" defaultChecked={model.servicos.chuva_dourada} />} label="Chuva dourada" />
                                            <FormControlLabel control={<Checkbox name="chuva_marrom" defaultChecked={model.servicos.chuva_marrom} />} label="Chuva marrom" />
                                            <FormControlLabel control={<Checkbox name="dominação" defaultChecked={model.servicos.dominacao} />} label="Dominação" />
                                            <FormControlLabel control={<Checkbox name="dupla_penetracao" defaultChecked={model.servicos.dupla_penetracao} />} label="Dupla penetração" />
                                            <FormControlLabel control={<Checkbox name="facefuck" defaultChecked={model.servicos.facefuck} />} label="Facefuck" />
                                            <FormControlLabel control={<Checkbox name="fisting" defaultChecked={model.servicos.fisting} />} label="Fisting" />
                                            <FormControlLabel control={<Checkbox name="masturbacao" defaultChecked={model.servicos.masturbacao} />} label="Masturbação" />
                                            <FormControlLabel control={<Checkbox name="massagem_tantrica" defaultChecked={model.servicos.massagem_tantrica} />} label="Massagem tântrica" />
                                            <FormControlLabel control={<Checkbox name="massagem_tradicional" defaultChecked={model.servicos.massagem_tradicional} />} label="Massagem tradicional" />
                                            <FormControlLabel control={<Checkbox name="oral_sem_preservativo" defaultChecked={model.servicos.oral_sem_preservativo} />} label="Oral sem preservativo" />
                                            <FormControlLabel control={<Checkbox name="penetração_com_acessorios" defaultChecked={model.servicos.penetração_com_acessorios} />} label="Penetração com uso de acessórios sexuais" />
                                            <FormControlLabel control={<Checkbox name="permite_filmagem" defaultChecked={model.servicos.permite_filmagem} />} label="Permite filmagem" />
                                            <FormControlLabel control={<Checkbox name="podolatria" defaultChecked={model.servicos.podolatria} />} label="Podolatria" />
                                            <FormControlLabel control={<Checkbox name="quirofilia" defaultChecked={model.servicos.quirofilia} />} label="Quirofilia" />
                                            <FormControlLabel control={<Checkbox name="roleplay" defaultChecked={model.servicos.roleplay} />} label="Roleplay" />
                                            <FormControlLabel control={<Checkbox name="sadomasoquismo" defaultChecked={model.servicos.sadomasoquismo} />} label="Sadomasoquismo" />
                                            <FormControlLabel control={<Checkbox name="sexo_anal_com_preservativo" defaultChecked={model.servicos.sexo_anal_com_preservativo} />} label="Sexo anal com preservativo" />
                                            <FormControlLabel control={<Checkbox name="sexo_com_voyeurismo" defaultChecked={model.servicos.sexo_com_voyeurismo} />} label="Sexo com voyeurismo / ser voyeur" />
                                            <FormControlLabel control={<Checkbox name="sexo_oral_com_preservativo" defaultChecked={model.servicos.sexo_oral_com_preservativo} />} label="Sexo oral com preservativo" />
                                            <FormControlLabel control={<Checkbox name="sexo_vaginal_com_preservativo" defaultChecked={model.servicos.sexo_vaginal_com_preservativo} />} label="Sexo vaginal com preservativo" />
                                            <FormControlLabel control={<Checkbox name="sexo_virtual" defaultChecked={model.servicos.sexo_virtual} />} label="Sexo virtual" />
                                            <FormControlLabel control={<Checkbox name="squirt" defaultChecked={model.servicos.squirt} />} label="Squirt" />
                                            <FormControlLabel control={<Checkbox name="striptease" defaultChecked={model.servicos.striptease} />} label="Striptease" />
                                            <FormControlLabel control={<Checkbox name="trampling" defaultChecked={model.servicos.trampling} />} label="Trampling" />
                                            <FormControlLabel control={<Checkbox name="tripla_penetracao" defaultChecked={model.servicos.tripla_penetracao} />} label="Tripla penetração" />
                                            <FormControlLabel control={<Checkbox name="uso_de_fantasias" defaultChecked={model.servicos.uso_de_fantasias} />} label="Uso de fantasias" />
                                        </FormGroup>
                                    </fieldset>
                                    <hr />
                                    <span className="flex justify-between">
                                ...
                                <div className='flex flex-col gap-5 w-full' style={{ display: formScreen == 'midia' ? 'flex' : 'none' }}>
                                    ...
                                    <fieldset className="flex flex-col md:flex-row gap-3">
                                        <TextField
                                            defaultValue={model.midia.facebook}
                                            label='Facebook'
                                            name='facebook'
                                            size='small'
                                            sx={{ flex: 1 }}
                                        />
                                        <TextField
                                            defaultValue={model.midia.instagram}
                                            label='Instagram'
                                            name='instagram'
                                            size='small'
                                            sx={{ flex: 1 }}
                                        />
                                        <TextField
                                            defaultValue={model.midia.tiktok}
                                            label='TikTok'
                                            name='tiktok'
                                            size='small'
                                            sx={{ flex: 1 }}
                                    ...
                                    <fieldset className="grid grid-cols-3 gap-3">
                                        <TextField
                                            defaultValue={model.midia.onlyfans}
                                            label='Onlyfans'
                                            name='onlyfans'
                                            size='small'
                                            sx={{ flex: 1 }}
                                        />
                                        <TextField
                                            defaultValue={model.midia.privacy}
                                            label='Privacy'
                                            name='privacy'
                                            size='small'
                                            sx={{ flex: 1 }}
                                    ...
                                    <span className="flex justify-between">
                                        ...
                                        <div className='flex gap-3'>
                                            ...
                                            <button className="px-5 py-2 border rounded-md hover:shadow-sm text-white bg-primary" type='submit'>Salvar</button>
                                        </div>
                                    </span>
                                </div>
                            </form>
                            {previewFloat &&
                                <div className='p-5 border flex flex-col gap-12 bg-white rounded-lg shadow-xl absolute mx-auto h-full min-w-[500px] z-50'>
                                    {preview == 'perfil' &&
                                        <>
